...
sys_abort_mtxColGrpItemsNotSame = function(x, name, columns, group_by, failed_groups){
  ...
  if(rlang::is_missing(name)){
    snd:::sys_abort(message = c("x" = "Inconsistent items in {.arg {arg}} columns, when grouped",
    ...
    snd:::sys_abort(message = c("x" = "Inconsistent items in {.mtx {mtx}} columns, when grouped",
                                "!" = "Please check in the following column:",
                                "!" = columns,
                                "i" = "Grouped by:",
                                "i" = group_by,
