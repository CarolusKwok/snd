...
sys_abort_mtxColContainNonFactor = function(x, name, columns){
  ...
  if(rlang::is_missing(name)){
    snd:::sys_abort(message = c("x" = "{.arg {arg}} contain non-factors in columns",
                                "!" = "Please correct the following columns:",
                                "!" = columns),
                    arg = rlang::caller_arg(arg = x))
  } else {
    snd:::sys_abort(message = c("x" = "{.mtx {name}} contain non-factors in columns",
                                "!" = "Please correct the following columns:",
                                "!" = columns),
